...
def int_to_bytes(n, length, endianess='little'):
	h = '%x' % n
	s = ('0'*(len(h) % 2) + h).zfill(length*2).decode('hex')
	return s[::-1] if endianess == 'little' else s
...
def salsa20_wordtobyte(inp):
	...
	x = [ord(i) for n in x for i in int_to_bytes(n, 4, 'little')]
	return x
...
def test_passes(i):
	...
	c_str = binascii.hexlify(b''.join([int_to_bytes(x, 1,'little') for x in c]))
